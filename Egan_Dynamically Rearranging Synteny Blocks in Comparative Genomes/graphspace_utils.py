...
def postBio131Graph(edge_list,node_attrs,graphid,directed=True):
	"""
	...
	if directed:
		edge_attrs = {}
		for e in edge_list:
			if e[0] not in edge_attrs:
				edge_attrs[e[0]] = {}
			edge_attrs[e[0]][e[1]] = {'target_arrow_shape':'triangle','target_arrow_fill':'filled','target_arrow_color':'black'}
		print(edge_attrs)

	else:
		edge_attrs = None
	data  = make_json_data(node_attrs.keys(),edge_list,node_attributes=node_attrs,edge_attributes=edge_attrs)
	write_json(data,jsonfile)
	postGraph(graphid,jsonfile,'compbio@reed.edu','compbio')
	shareGraph(graphid,'compbio@reed.edu','compbio','Bio131Spring2017HW6','compbio@reed.edu')
	return
